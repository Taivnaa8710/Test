...
base_path = 'Dataset/Multiple_dynamic_images_stride_10_frames_15'
...
with open(os.path.join(base_path, 'celebdf.json')) as metadata_json:
...
for filename in metadata.keys():
	...
	faces_path = os.path.join(tmp_path, 'faces380x380')
	...
	for frame in frame_images:
		...
		for result in results:
			...
			if len(results) < 2 or confidence > 0.95:
				margin_x = bounding_box[2] * 0.6  # 60% as the margin
				...
				resized_image = cv2.resize(crop_image,(380,380))
				new_filename = '{}-{:02d}.jpg'.format(os.path.join(faces_path, get_filename_only(frame)), count)
				count = count + 1
				cv2.imwrite(new_filename, cv2.cvtColor(resized_image, cv2.COLOR_RGB2BGR))
			else:
